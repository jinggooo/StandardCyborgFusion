...
struct ICPConfiguration {
    float tolerance = 1e-9; // if the relative correspondence error is below this tolerance value, then the ICP is done.
    int maxIterations = 144; // maximum number of iterations to run ICP.
    float outlierDeviationsThreshold = 1.0; // threshold value, used for filtering out outlier points.
    int threadCount = 8; // number of threads allowed to use, for the correspondence search of ICP.
};
